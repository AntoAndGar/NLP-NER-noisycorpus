...
class NERv2(nn.Module):
    ...
    def init_weights(self, w, freeze=False):
        ...
        return torch.nn.Embedding.from_pretrained(vectors, freeze=freeze, padding_idx=self.voc_size-1)
    ...
    def forward(self, input) -> torch.Tensor:
        ...
        if len(embedding.shape) < 3:
            ...
            #initialize hidden and cell states
            h0 = torch.ones(self.bidirectional*self.num_layers, embedding.shape[0], self.hidden_size).to(self.device)
            c0 = torch.ones(self.bidirectional*self.num_layers, embedding.shape[0], self.hidden_size).to(self.device)
